...
async fn test_dry_run_execution() {
    ...
    match client.start().await {
        ...
        Err(e) => {
            eprintln!(
                "Failed to start client (likely need to build mcp-server-trade first): {}",
                e
            );
            return;
        }
    ...
    match response {
        Ok(res) => {
            ...
            assert!(
                res.contains("DryRun"),
                "Response should indicate DryRun mode"
            );
            assert!(
                res.contains("dry_run_id"),
                "Response should contain dry run ID"
            );
        }
