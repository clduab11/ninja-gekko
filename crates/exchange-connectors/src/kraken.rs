...
use crate::{
    ExchangeConnector, ExchangeError, ExchangeId, ExchangeResult, ExchangeOrder,
    OrderSide, OrderType, OrderStatus, TradingPair, Balance, MarketTick,
    StreamMessage, TransferRequest, TransferStatus, RateLimiter,
    Timeframe, Candle, Fill,
...
impl KrakenConnector {
    ...
    fn generate_signature(&self, path: &str, nonce: &str, post_data: &str) -> ExchangeResult<String> {
        let secret = self.credentials.api_secret();
        
        // 1. Decode API Secret
        let secret_bytes = STANDARD.decode(secret)
            .map_err(|e| ExchangeError::Configuration(format!("Invalid Kraken API Secret (Base64): {}", e)))?;
    ...
    async fn send_private_request<T>(&self, path: &str, params: &mut HashMap<&str, String>) -> ExchangeResult<T>
    where T: for<'de> Deserialize<'de>
    {
        self.rate_limiter.acquire().await?;
        ...
        let post_data = serde_urlencoded::to_string(&params)
            .map_err(|e| ExchangeError::InvalidRequest(format!("Failed to serialize params: {}", e)))?;
    ...
    where T: for<'de> Deserialize<'de>
    {
        ...
        let json: Value = serde_json::from_str(&text)
            .map_err(|e| ExchangeError::InvalidRequest(format!("JSON parse error: {} Body: {}", e, text)))?;

        if let Some(errors) = json.get("error").and_then(|e| e.as_array()) {
            if !errors.is_empty() {
                let error_msgs: Vec<String> = errors.iter().map(|v| v.as_str().unwrap_or("unknown").to_string()).collect();
                return Err(ExchangeError::Api {
...
impl ExchangeConnector for KrakenConnector {
    ...
    async fn connect(&mut self) -> ExchangeResult<()> {
        ...
        if !self.credentials.api_key().is_empty() {
            let _: HashMap<String, Value> = self.send_private_request("/0/private/Balance", &mut HashMap::new()).await?;
        }
    ...
    async fn get_trading_pairs(&self) -> ExchangeResult<Vec<TradingPair>> {
        let result: HashMap<String, Value> = self.send_public_request("/0/public/AssetPairs", &[]).await?;
            ...
            if let Some(base) = info.get("base").and_then(|v| v.as_str()) {
                if let Some(quote) = info.get("quote").and_then(|v| v.as_str()) {
                    ...
                    pairs.push(TradingPair {
                        ...
                        symbol: info.get("altname").and_then(|v| v.as_str()).unwrap_or(&name).to_string(),
                    });
                }
            }
        }
    ...
    async fn get_balances(&self) -> ExchangeResult<Vec<Balance>> {
        let balances_map: HashMap<String, String> = self.send_private_request("/0/private/Balance", &mut HashMap::new()).await?;
    ...
    async fn place_order(
        ...
        params.insert("type", match side {
            OrderSide::Buy => "buy".to_string(),
            OrderSide::Sell => "sell".to_string(),
        });
        params.insert("ordertype", match order_type {
        ...
        let result: PlaceOrderResponse = self.send_private_request("/0/private/AddOrder", &mut params).await?;
    ...
    async fn cancel_order(&self, order_id: &str) -> ExchangeResult<ExchangeOrder> {
        ...
        let result: CancelOrderResponse = self.send_private_request("/0/private/CancelOrder", &mut params).await?;
    ...
    async fn get_order(&self, order_id: &str) -> ExchangeResult<ExchangeOrder> {
        ...
        let result: HashMap<String, Value> = self.send_private_request("/0/private/QueryOrders", &mut params).await?;
        
        // Parse complex result to ExchangeOrder...
        // This is complex because result is map of txid -> order info
        Err(ExchangeError::InvalidRequest("Not fully implemented yet".to_string()))
    }

    async fn get_market_data(&self, symbol: &str) -> ExchangeResult<MarketTick> {
        let result: HashMap<String, Value> = self.send_public_request("/0/public/Ticker", &[("pair", symbol)]).await?;
        
        // Result keys are pair names, which might differ slightly from requested symbol if alias used
        // We just take the first value
        let (_key, val) = result.iter().next().ok_or_else(|| ExchangeError::OrderNotFound("No ticker data".to_string()))?;
        ...
        let ask = val.get("a").and_then(|a| a.get(0)).and_then(|v| v.as_str()).unwrap_or("0");
        let bid = val.get("b").and_then(|b| b.get(0)).and_then(|v| v.as_str()).unwrap_or("0");
        let last = val.get("c").and_then(|c| c.get(0)).and_then(|v| v.as_str()).unwrap_or("0");
        let vol = val.get("v").and_then(|v| v.get(1)).and_then(|v| v.as_str()).unwrap_or("0"); // 24h vol
    ...
    async fn start_market_stream(
        ...
        Err(ExchangeError::InvalidRequest("Kraken WS not implemented yet".to_string()))
    }
    ...
    async fn transfer_funds(&self, _request: TransferRequest) -> ExchangeResult<String> {
         Err(ExchangeError::InvalidRequest("Transfer not supported".to_string()))
    }

    async fn get_transfer_status(&self, _transfer_id: &str) -> ExchangeResult<TransferStatus> {
        Err(ExchangeError::InvalidRequest("Transfer status not supported".to_string()))
    }
    ...
    async fn get_candles(
        ...
        let result: HashMap<String, Value> = self.send_public_request("/0/public/OHLC", &[("pair", symbol), ("interval", interval)]).await?;
            ...
            if let Some(arr) = val.as_array() {
                for c in arr {
                    ...
                    if let (Some(time), Some(open), Some(high), Some(low), Some(close), Some(vol)) = (
                        ...
                        candles.push(Candle {
                            start_time: chrono::DateTime::from_timestamp(time, 0).unwrap_or_else(chrono::Utc::now),
