...
#[tokio::test]
async fn verify_coinbase_transaction_flow() -> anyhow::Result<()> {
    // 1. Setup Logging
    let subscriber = FmtSubscriber::builder()
        .with_max_level(Level::INFO)
        .finish();
    if let Err(_) = tracing::subscriber::set_global_default(subscriber) {
        // Ignore if already set (e.g. by test runner)
    }

    info!("🚀 Starting Coinbase Transaction Flow Verification");
    ...
    println!("DEBUG: PRIVATE_KEY found (len={}): {}...", private_key.len(), &private_key.chars().take(20).collect::<String>());

    let config = CoinbaseConfig {
        api_key_name,
        private_key,
        sandbox: false, // We want to verify REAL credentials, be CAREFUL with orders.
    ...
    // Minimum valid size. For BTC-USDC, min base size is 0.00001 (satoshis usually allow small)
    // Let's use 0.0001 BTC to be safe.
