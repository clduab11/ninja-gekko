...
import { fetchNews, fetchAccountSnapshot, fetchHistoricalCandles } from '../../services/api';

/**
 * Type definitions for Exchange Trading Instrument and Exchange
 */
export type TradingInstrument = {
  /** Standardized symbol, e.g. 'EUR/USD', 'BTCUSD', etc. */
  symbol: string;
  /** Human-friendly instrument name, e.g. 'Euro', 'Bitcoin (CFD)' */
  name: string;
  /** Optional metadata for advanced display or API integrations */
  category?: 'forex' | 'crypto' | 'cfd';
...
export type ExchangeConfig = {
  ...
  id: string;
  /** Display name (e.g. 'Binance.US', 'OANDA', etc.) */
  name: string;
  /** Type of market (e.g. 'SPOT', 'FX/CFD') */
  type: string;
  /** List of supported trading instruments */
  pairs: TradingInstrument[];
};

/**
 * Sample data structure for exchanges and their supported trading pairs.
 * This should be refactored out for production to a config or API source.
 */
...
const MarketRadar = () => {
  ...
  const [symbol, setSymbol] = React.useState(EXCHANGES['COINBASE'].pairs[0].symbol);
  const [timeframe, setTimeframe] = React.useState('15m');

  const { data: news, isLoading: isNewsLoading, isError: isNewsError, error: newsError } = useQuery({ queryKey: ['news'], queryFn: fetchNews, refetchInterval: 30000 });
  const { data: snapshot } = useQuery({ queryKey: ['account-snapshot'], queryFn: fetchAccountSnapshot });
  
  const { data: historicalData, isLoading: isChartLoading, isError: isChartError, error: chartError } = useQuery({
