...
use crate::{
    ...
    models::{
        ApiResponse, MarketDataPoint, MarketDataRequest, MarketDataResponse,
        MarketDataWithIndicators, MarketOverview, MarketStatistics, PaginatedResponse,
        PaginationParams, SearchSymbolsRequest, StreamSubscriptionResponse, SymbolInfo,
    },
    AppState,
};
...
pub async fn get_market_data(
    ...
    match state.market_data_service.get_latest_data(&symbol).await {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve market data: {}", e),
            })
        }
    }
}
...
pub async fn get_batch_market_data(
    ...
    info!(
        "Retrieving batch market data for symbols: {:?}",
        request.symbols
    );

    if request.symbols.is_empty() {
        return Err(ApiError::Validation {
            message: "Symbols list cannot be empty".to_string(),
            field: Some("symbols".to_string()),
        });
    }

    match state
        .market_data_service
        .get_batch_data(&request.symbols)
        .await
    {
        Ok(data_list) => Ok(Json(ApiResponse::success(data_list))),
        Err(e) => {
            warn!("Failed to retrieve batch market data: {}", e);
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve batch market data: {}", e),
            })
        }
    }
}
...
pub async fn get_historical_data(
    ...
    info!(
        "Retrieving historical data for symbol: {} with params: {:?}",
        symbol, params
    );
    ...
    match state
        .market_data_service
        .get_historical_data(&symbol, params)
        .await
    {
        Ok(history) => {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve historical data: {}", e),
            })
        }
    }
}
...
pub async fn get_price_with_indicators(
    ...
    info!(
        "Retrieving price data with indicators for symbol: {}",
        symbol
    );

    match state
        .market_data_service
        .get_data_with_indicators(&symbol, params)
        .await
    {
        Ok(data) => Ok(Json(ApiResponse::success(data))),
        Err(e) => {
            warn!(
                "Failed to retrieve price with indicators for {}: {}",
                symbol, e
            );
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve price with indicators: {}", e),
            })
        }
    }
}
...
pub async fn search_symbols(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to search symbols: {}", e),
            })
        }
    }
}
...
pub async fn get_market_overview(
    ...
    match state.market_data_service.get_market_overview().await {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve market overview: {}", e),
            })
        }
    }
}
...
pub async fn get_price_stream(
    ...
    match state
        .market_data_service
        .subscribe_to_price_stream(&symbol)
        .await
    {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to start price stream: {}", e),
            })
        }
    }
}
...
pub async fn get_market_statistics(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            Err(ApiError::MarketData {
                message: format!("Failed to retrieve market statistics: {}", e),
            })
        }
    }
}
