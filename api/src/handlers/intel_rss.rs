...
fn determine_sentiment(title: &str) -> f32 {
    ...
    let bullish_keywords = [
        "surge", "rally", "soar", "bullish", "gains", "rise", "jump", "moon", "record", "high",
        "adoption", "approval",
    ];
    let bearish_keywords = [
        "crash", "fall", "drop", "bearish", "loss", "plunge", "fear", "sell", "dump", "low",
        "reject", "ban",
    ];

    let bullish_count = bullish_keywords
        .iter()
        .filter(|k| title_lower.contains(*k))
        .count();
    let bearish_count = bearish_keywords
        .iter()
        .filter(|k| title_lower.contains(*k))
        .count();
...
pub async fn start_rss_polling_task(cache: Arc<RwLock<RssCache>>, poll_interval_secs: u64) {
    ...
    loop {
        ...
        info!(
            "RSS Poller: Cache updated, sleeping for {}s",
            poll_interval_secs
        );
